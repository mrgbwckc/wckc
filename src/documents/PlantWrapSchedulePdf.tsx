...
export const PlantWrapSchedulePdf = ({
  data,
  dateRange,
}: {
  data: PlantTableView[];
  ...
  const grouped = data.reduce((acc, row) => {
    const dateKey = row.wrap_date
      ? dayjs(row.wrap_date).format("YYYY-MM-DD")
      : "Unscheduled";
    ...
    <Document>
      <Page size="A4" orientation="landscape" style={styles.page}>
        <View style={styles.headerContainer}>
          <Text style={styles.reportTitle}>Plant Wrap Schedule</Text>
          <View>
